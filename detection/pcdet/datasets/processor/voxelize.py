...
def voxelize_with_rec_idx(coord, voxel_size=0.05, hash_type='fnv', mode=0):
    discrete_coord = np.floor(coord / np.array(voxel_size))
    if hash_type == 'ravel':
        key = ravel_hash_vec(discrete_coord)
    else:
    ...
    if mode == 0:  # train mode
        ...
        idx_recon = np.zeros(key.shape[0]).astype(np.int32)
        idx_recon[idx_sort] = sorted_idx
        return idx_unique, idx_recon


def voxelize_with_rec_idx_v2(coord, voxel_size=0.05, hash_type='fnv'):
    discrete_coord = np.floor(coord / np.array(voxel_size))
    if hash_type == 'ravel':
        key = ravel_hash_vec(discrete_coord)
    else:
...
# def voxelize_with_rec_idx_meanvoxel(coord, xyz, feat, voxel_size=0.05, hash_type='fnv', mode=0):
#     discrete_coord = np.floor(coord / np.array(voxel_size))
#     if hash_type == 'ravel':
#         key = ravel_hash_vec(discrete_coord)
#     else:
